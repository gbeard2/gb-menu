...
name = textbox.Textbox(text='Name: Unknown')
# Add the textbox to the menu
main_menu.add_textbox(name)
change_name_action = action.Action(function=change_name)
change_name_choice = choice.Choice(text='Change name', action=change_name_action)
...
runtime = textbox.Textbox(text='Runtime: 0 s', pos=5, update_on_show=True, update_action=runtime_action)
